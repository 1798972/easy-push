...
public abstract class MessageProvider {
    ...
    protected MessageContext message2MessageContext(Message message) {
        final String split = "-";
        return new MessageContext.Builder().id(UUID.randomUUID().toString().replace(split, "")).messageSceneType(message.getMessageSceneType()).timestamp(String.valueOf(System.currentTimeMillis())).message(message).build();
    }
